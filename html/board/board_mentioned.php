...
<html>
    ...
    <body class="frame">
        ...
        <div class="article_list">
            <?php
                $sql = "SELECT message_id as m_id, content, nickname, hit, message_time 
                        FROM `message` NATURAL JOIN mention NATURAL JOIN `user`
                        WHERE mentioned_user = '$ID'
                        AND writer_id = `user_id`
                        AND parent_message is null
                            UNION
                        SELECT parent_message as m_id, content, nickname, hit, message_time 
                        FROM `message` NATURAL JOIN mention NATURAL JOIN `user`
                        WHERE mentioned_user = '$ID'
                        AND writer_id = `user_id`
                ...
                while($board=mysqli_fetch_array($result1)){
                    ...
                    $url="./read/read.php?id=".$board['m_id'];
